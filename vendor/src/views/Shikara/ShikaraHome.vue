    ...
    <v-layout wrap justify-center pa-0 pa-sm-4>
      <v-flex xs12>
        <v-layout wrap justify-center py-2>
          ...
          <v-spacer></v-spacer>
          <v-flex xs12 sm3 lg2 xl1 align-self-center px-1>
            <v-text-field
              v-model="keyword1"
              dense
              placeholder="Search"
          ...
          <v-flex xs12 sm3 lg2 xl1 align-self-center pt-2 pt-sm-0>
            <v-btn dense block class="gr1" @click="$router.push('/AddShikara')"
              ><span style="color: white">Add New Shikara</span></v-btn
            >
          </v-flex>
        </v-layout>
                  ...
                  >
                    <v-layout wrap>
                      <v-flex
                        ...
                        @click="$router.push('/ViewShikara?id=' + item._id)"
                      >
                        <v-img
                          v-if="item.coverImage"
                          height="90px"
                          ...
                          <v-flex xs12 pt-2>
                            <span
                              style="
                                ...
                                font-family: LexendFont;
                              "
                              >No of Seats : {{ item.totalSeats }}</span
                            >
                          </v-flex>
                          <v-flex xs12 pt-2 v-if="item.shikaraStatus">
                            <span
                      ...
                      <v-flex xs1 v-if="item.shikaraStatus != 'Blocked'">
                        <v-menu bottom left>
                          ...
                          <v-list>
                            ...
                            <v-list-item
                              @click="
                                $router.push('/ViewShikara?id=' + item._id)
                              "
                            >
                              <v-list-item-title
                    ...
                    <v-layout wrap pt-4 pb-2>
                      <v-flex xs12 pt-8 text-center>
                        <v-btn
                          v-if="item.isComplete == false"
                          dense
                          block
                          :disabled="
                            item.shikaraStatus === 'Blocked' ? true : false"
                          @click="
                            $router.push(
                              '/AddDetail?name=' +
                                item.shikaraName +
                        ...
                        <v-btn
                          ...
                          :disabled="
                            item.shikaraStatus === 'Blocked' ? true : false"
                          @click="
                            $router.push('/tripEditShikara?id=' + item._id)
                          "
                          color="#f17343"
                          style="border-radius: 10px"
                          ><span
                      ...
                      <v-flex xs12 pt-2 text-center>
                        <v-btn
                          ...
                          :disabled="item.shikaraStatus === 'Blocked' ||item.hasPackage == false ? true : false"
                          @click="
                            $router.push(
                              '/ShikaraReservation?name=' +
                                item.shikaraName +
                                '&&id=' +
                                item._id
                            )
