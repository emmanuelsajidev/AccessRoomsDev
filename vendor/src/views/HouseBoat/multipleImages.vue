  ...
  <div>
    ...
    <v-layout>
      <v-flex xs12 md12 align-self-center>
        ...
        <span class="title3" style="color: #505050">Add more images</span>
        <input
          v-show="false"
          accept="image/*"
          id="file"
    ...
    <v-layout wrap pa-2>
      <template v-if="imageArray.length > 0">
        <v-flex xs2 v-for="(item, index) in imageArray" :key="index">
          <v-badge
          ...
          <v-badge icon="mdi-check-circle-outline" overlap id="myIcon">
            <template slot="badge">
              <v-dialog v-model="item.delete" max-width="600px">
                ...
                <v-card>
                  <v-card-title
                    >Are you sure you want to delete this Photo?</v-card-title
                  >
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn
