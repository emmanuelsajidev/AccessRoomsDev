...
async function CalculateNightStayRate(houseboat,dates,noOfGuest){
    try{
        var totalAmount = 0;
        ...
        for(var i=0;i<dates.length;i++){
            ...
            var checkForSpecialRate = await tripPackageModel.findOne({status:"Active",packageType:"Special",houseBoatId:houseboat._id,tripTypeId:houseboat.tripDetails._id,startDate:{$gte:DateStartEnd.dStart},endDate:{$lte:DateStartEnd.dEnd,},agentRate:{$gte:0}})
            if(Misc.isnullorempty(checkForSpecialRate)){
                ...
                var details = await tripPackageModel.aggregate([
                    {$match:{packageType:{$ne:"Special"},status:"Active",houseBoatId:new mongoose.Types.ObjectId(houseboat._id),tripTypeId:new mongoose.Types.ObjectId(houseboat.tripDetails._id),startDate:{$lte:startDate},endDate:{$gte:endDate},agentRate:{$gte:0}}}
                ])
                if(details.length<=0){
                ...
                var totalGuestsCanBeAccomadated = totalRoomsAvailable*houseboat.tripDetails.maxPersonPerRooms
                var totalExtraGuestsCanBeAccomadated = totalRoomsAvailable*houseboat.tripDetails.extraPersonPerRooms
...
async function CalculateDayCruisRate(houseboat,dates,noOfGuest){
    try{
        var totalAmount = 0;
        var totalCommissionAmount=0;
        var isValid = true;
        for(var i=0;i<dates.length;i++){
            ...
            if(Misc.isnullorempty(checkForSpecialRate)){
                ...
                var minGuestNeeded = houseboat.tripDetails.minCapacityOfBoats;
                var maxGuestNeeded = houseboat.tripDetails.maxCapacityOfBoats;
                var totalGuests = noOfGuest
                var ratePerHead = normalRate.agentRate;
                var ratePerHeadComission = normalRate.agentRateCommission
...
async function CalculateOverNightStayRate(houseboat,dates,noOfGuest){
    try{
        var totalAmount = 0;
        var totalCommissionAmount=0;
        var isValid = true;
...
async function CalculateNightStayRateSharing(houseboat,dates,noOfGuest,totalRooms){
    try{
        var totalAmount = 0;
        var totalCommissionAmount=0;
        var isValid = true;
...
async function CalculateOverNightStayRateCombination(houseboat,dates,noOfGuest){
    try{
        var totalAmount = 0;
        var totalCommissionAmount=0;
        var isValid = true;
...
async function CalculateNightStayRateCombination(houseboat,dates,noOfGuest){
    try{
        var totalAmount = 0;
        var totalCommissionAmount=0;
        var isValid = true;
